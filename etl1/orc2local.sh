...
sql="set hive.cli.print.header=false;use ${db_name}; select coalesce(country,'all'),coalesce(substring(uptime,1,8),'all'),count(*) from ${orc_name1} where month='${month}' and day='${day}' group by country,substring(uptime,1,8) with cube;"
...
sql="set hive.cli.print.header=false;use ${db_name}; select coalesce(ip,'all'),coalesce(substring(uptime,1,8),'all'),count(*) from ${orc_name2} where month='${month}'and day='${day}' group by ip,substring(uptime,1,8) with cube;"
